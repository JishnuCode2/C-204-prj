...
def createTicket(): 
    global gameWindow 
    global ticketGrid #Ticket Frane
    ...
    for row in range(0, 3):
        ...
        for col in range(0, 9):
            if(platform.system() == 'Darwin'):
                boxButton = mac_button(gameWindow, font = ("Chalkboard SE",18), borderwidth=3,pady=23,padx=-22,bg="#1ff176",highlightbackground="#fff176",activebackground='#c5ela5')
                boxButton.place(x=xPos, y=yPos)
            else: 
                boxButton = Button(gameWindow, font=("Chalkboard SE",15), width=2, height=1, borderwidth=5, bg="orange")
                boxButton.place(x=xPos, y=yPos)
...
def placeNumbers():
    ...
    while row < 3:
        ...
        counter_ = 0
        while counter_ < len(randomColList):
            colNum = randomColList[counter_]
            ...
            if(randomNumber not in currentNumberList):
                ...
                print(ticketGrid[row][colNum])
                numberBox.configure(text=randomNumber, fg='black', bg='yellow')
                currentNumberList.append(randomNumber)
                counter_ += 1 

        row += 1
        rowList.append(randomColList)
...
def game_window():
    ...
    canvas2 = Canvas(gameWindow, width = 800,height = 500)
    ...
    # Flash Number Label
    flashNumberLabel = canvas2.create_text(400,screen_height/2+30, text = "Waiting for other players to join...", font=("Chalkboard SE",30), fill="#3e2723")
...
def askPlayerName():
    ...
    button = Button(nameWindow, text="Save", font=("Chalkboard SE", 30),width=14, command=saveName, height=1, bg="#80deea", bd=3)
    button.place(x = screen_width/6-30, y=screen_height/4+20)
